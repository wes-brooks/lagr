lagr.cv.f = function(formula, data, weights, family, bw, coords, kernel, env, oracle, varselect.method, verbose, longlat, tol.loc, bw.type, N, parallel, interact, bwselect.method, resid.type) {    
    ...
    if (bwselect.method=='AICc') {
        ...
        loss = trH + sum(sapply(lagr.model[['model']][['models']], function(x) {min(x[['tunelist']][['ssr-loc']][[resid.type]])}))
        #"Simplistic" BIC - based on eq4.22 from the Fotheringham et al. book:
        #loss = nrow(data) * (log(mean(sapply(lagr.model[['model']][['models']], function(x) {x[['ssr.local']]}))) + 1 + log(2*pi)) + trH * log(nrow(data))/2
    }
