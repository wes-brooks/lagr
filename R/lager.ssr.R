lager.ssr = function(bw, x, y, family, coords, loc, dist, verbose, prior.weights, kernel, target, varselect.method, interact, oracle, resid.type) {
  #Calculate the local weights:
  kernel.weights = drop(kernel(dist, bw))
  
  lager.object = lager.fit.inner(x=x,
                                 y=y,
                                 family=family,
                                 coords=coords,
                                 loc=loc,
  ...
  loss = lager.object[['ssr']][[resid.type]]
